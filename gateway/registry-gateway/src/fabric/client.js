...
let gateway = null, contract = null, grpcClient = null;
...
async function proposeAnchor(assetId, payloadHash, payloadPtr, poseSummary, qualitySummary, publisherId) {
    ...
    logger.info('Submitting to chaincode:', { 
        assetId, 
        payloadHash, 
        payloadPtr, 
        poseSummaryStr, 
    ...
    const resultStr = uint8ArrayToString(result);
    logger.info('Chaincode result:', resultStr);
...
async function endorseAnchor(claimId, endorserId) {
    ...
    const resultStr = uint8ArrayToString(result);
    logger.info('Endorse result:', resultStr);
    return JSON.parse(resultStr);
}
...
module.exports = { initializeFabric, closeFabric, getContract, isInMockMode, proposeAnchor, endorseAnchor, resolveAnchor, revokeAnchor, getClaim, listClaims };
