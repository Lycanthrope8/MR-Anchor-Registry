...
document.addEventListener('DOMContentLoaded', () => {
    ...
    refreshAll();
    wireEvents();
    logEvent('system', 'Org2 Admin Panel initialized (v2.0 — annotations enabled)');
});
...
async function refreshPending() {
    ...
    try {
        ...
        // Action required section (Org2 hasn't endorsed)
            ...
            myBody.innerHTML = myRevocations.length === 0 
                ...
                : myRevocations.map(r => `
                    <tr>
                        ...
                        <td><span class="badge warn">Waiting for Org1</span></td>
                    </tr>
                `).join('');
        }
        
        // Their revocations (Org1 initiated, I need to respond)
...
async function endorseClaim(assetId) {
    ...
    try {
        ...
        if (result.is_fully_endorsed) {
            ...
            showToast('✓ Endorsed. Waiting for Org1...', 'success');
        }
        refreshAll();
    } catch (error) {
...
async function endorseAnnotation(assetId) {
    if (!confirm(`Endorse annotation for ${assetId}?\n\nThis adds Org2's approval. The annotation becomes ACTIVE once both organizations endorse.`)) return;
    ...
    try {
        ...
        if (result.is_fully_endorsed) {
            ...
            showToast('✓ Annotation endorsed. Waiting for Org1...', 'success');
        }
        refreshAnnotations();
    } catch (error) {
        showToast(`Error: ${error.message}`, 'error');
